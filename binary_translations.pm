...
sub data {
    return (
        ...
        "[000000]作戦海域への出撃時に、マップ上で [006400]Lボタン[-]\n切り替え\rによって、戦闘演出をカットして\n戦闘時間を短縮する\r[0055aa]戦闘ショートカット[-]が\n可能です。\n\n出撃&戦闘\rに慣れてきたら、ご活用ください。\nただし、未攻略海域の最深部、敵主力艦隊\nとの交戦は、戦闘ショートカットできません。[-]" =>{ tr => "When you are used to how a map works, you may use Battle Shortcut. By pressing the L-Shoulder button during on the Sortie Map, you can shorten battle times. However, in fleet battles with new areas, Battle Shortcut may not be used.",      desc => '', ok=>'resources/resources_01570.-10 196'},
        ...
        "艦隊は遠征中です"=>{ tr => "",       desc => "UIShortCutMenu", ok => 'a-csharp 4495756'},
        "艦隊を編成する必要があります"=>{ tr => "",      desc => "UIPortFrame, UIShortCutMenu", ok => 'a-csharp 4495810'},
        "撤退中の艦が含まれています"=>{ tr => "",      desc => "UIShortCutMenu", ok => 'a-csharp 4495840'},
        "鎮守府海域でのみ選択可能です"=>{ tr => "",      desc => "UIShortCutMenu", ok => 'a-csharp 4495868'},
        "旗艦が工作艦である必要があります"=>{ tr => "",      desc => "UIShortCutMenu", ok => 'a-csharp 4495898'},
        "決戦！ショートカット不可"=>{ tr => "",      desc => "UIShortCutSwitch", ok => 'a-csharp 4496090'},
        "戦闘ショートカット"=>{ tr => "",      desc => "UIShortCutSwitch", ok => 'a-csharp 4496070'},
        "開発資材のインデックスが不正です"=>{ tr => "",      desc => "Arsenal.Arsenal_DevKit", ok => 'a-csharp 4610075' },
        "戦略ポイントのインデックスが不正です"=>{ tr => "",      desc => "Arsenal.Arsenal_SPoint", ok => 'a-csharp 4610109' },
        "輸送船を"=>{ tr => "",      desc => "Arsenal.ArsenalTankerDialog"                 },
        "隻入手しました"=>{ tr => "",      desc => "Arsenal.ArsenalTankerDialog", ok => 'a-csharp 4516794'},
        "艦がロックされています"=>{ tr => "",      desc => "Arsenal.TaskArsenalListManager", ok => 'a-csharp 4517094'},
        "ロックされている装備があります"=>{ tr => "",      desc => "Arsenal.TaskArsenalListManager", ok => 'a-csharp 4517118'},
        "建造"=>{ tr => "",      desc => ".Arsenal.TaskConstructManager",},
        "大型建造"=>{ tr => "",      desc => ".Arsenal.TaskConstructManager", ok => 'a-csharp 4517764'},
        "開発"=>{ tr => "",      desc => ".Arsenal.TaskConstructManager",},
        "輸送船建造"=>{ tr => "",      desc => ".Arsenal.TaskConstructManager",},
        "戦略ポイントが不足しています"=>{ tr => "",      desc => ".Arsenal.TaskConstructManager", ok => 'a-csharp 4518318'},
        "解体  艦種  艦名　　　　　      Lv"=>{ tr => "",      desc => "Arsenal.TaskMainArsenalManager", ok => 'a-csharp 4518706'},
        "廃棄      装備名　　　　　　　　　 レア"=>{ tr => "",      desc => "Arsenal.TaskMainArsenalManager", ok => 'a-csharp 4518754'},
        "燃料"=>{ tr => "",      desc => "Arsenal.UiArsenalMaterialDialog",},
        "弾薬"=>{ tr => "",      desc => "Arsenal.UiArsenalMaterialDialog",},
        "鋼材"=>{ tr => "",      desc => "Arsenal.UiArsenalMaterialDialog",},
        "ボーキサイト"=>{ tr => "",      desc => "Arsenal.UiArsenalMaterialDialog",},
        "高速建造材"=>{ tr => "",      desc => "Arsenal.UIHowToArsenal",},
        "随伴艦の"=>{ tr => "",      desc => "Battle.Production.ProdEscortShipEvacuation", ok => 'a-csharp 4557329'},
        "を護衛につけて戦場から退避させますか？"=>{ tr => "",      desc => "Battle.Production.ProdEscortShipEvacuation", ok => 'a-csharp 4557339'},
        "『{0}』艦隊<br>旗艦「{1}」が<br>大破しました。"=>{ tr => "",      desc => "Battle.Production.ProdFlagshipWreck", ok => ['a-csharp 4557605', 'a-csharp 4698449']},
        "進撃は困難です……帰投します。"=>{ tr => "",      desc => "Battle.Production.ProdFlagshipWreck", ok => ['a-csharp 4557685', 'a-csharp 4698510']},
        "敵反攻作戦 迎撃成功!!"=>{ tr => "",      desc => "Battle.Production.ProdThalassocracy", ok => 'a-csharp 4558267'},
        "「{0}」リクエスト中♪"=>{ tr => "",      desc => "Furniture.JukeBox.UIJukeBoxMusicPlayingDialog", ok => 'a-csharp 4532096'},
        "家具コインが不足しています"=>{ tr => "",      desc => "Furniture.JukeBox.UserInterfaceJukeBoxManager", ok => 'a-csharp 4532184'},
        "購入 - {0} - "=>{ tr => "",      desc => "InteriorStore.UIFurniturePurchaseDialog", ok => 'a-csharp 4525706'},
        "不要"=>{ tr => "",      desc => "InteriorStore.UIFurniturePurchaseDialog", ok => 'a-csharp 4525736'},
        "必要"=>{ tr => "",      desc => "InteriorStore.UIFurniturePurchaseDialog",},
        "給糧艦「間宮」を使用しますか？"=>{ tr => "",      desc => "Organize.OrganizeTender", ok => 'a-csharp 4529162'},
        "給糧艦「伊良湖」を使用しますか？"=>{ tr => "",      desc => "Organize.OrganizeTender", ok => 'a-csharp 4529302'},
        "艦隊名を入力してください。（12文字まで）"=>{ tr => "",      desc => "Organize.TaskOrganizeTop", ok => 'a-csharp 4532034'},
        "「"=>{ tr => "",      desc => "Production.ProdBattleRewardItem",},
        "」を" =>{ tr => "",      desc => "Production.ProdBattleRewardItem",},
        "入手しました。"=>{ tr => "",      desc => "Production.ProdBattleRewardItem", ok => 'a-csharp 4492312'},
        "」を入手しました。" =>{ tr => "",      desc => "Production.ProdBattleRewardItem", ok => 'a-csharp 4492328'},
        " 装備の開発に失敗しました。\n\n『開発資材』は消費しませんでした。" =>{ tr => "",      desc => "Production.ProdReceiveSlotItem", ok => 'a-csharp 4492850'},
        "部隊再編中…"=>{ tr => "",      desc => "Production.ProdRevampReceiveItem", ok => 'a-csharp 4493086'},
        "装備改修中…"=>{ tr => "",      desc => "Production.ProdRevampReceiveItem", ok => 'a-csharp 4493100'},
        "{0}に装備が改修更新されました！"=>{ tr => "",      desc => "Production.ProdRevampReceiveItem", ok => 'a-csharp 4493138'},
        "{0}に部隊再編完了！"=>{ tr => "",      desc => "Production.ProdRevampReceiveItem", ok => 'a-csharp 4493114'},
        "艦が保有上限に達しています" =>{ tr => "",      desc => "Scene.Duty.UserInterfaceDutyManager", ok => 'a-csharp 4524382'},
        "装備が保有上限に達しています"=>{ tr => "",      desc => "Scene.Duty.UserInterfaceDutyManager", ok => 'a-csharp 4524410'},
        "該当装備がロックされています" =>{ tr => "",      desc => "Scene.Duty.UserInterfaceDutyManager", ok => 'a-csharp 4524352'},
        "撤退中の艦を含んでいます"=>{ tr => "",      desc => "Strategy.DeckStateViews", ok => 'a-csharp 4567087'},
        "補給対象の艦が居ません" =>{ tr => "",      desc => "Strategy.DeckStateViews", ok => 'a-csharp 4567113'},
        "資源が不足しています"  =>{ tr => "",      desc => "Strategy.DeckStateViews", ok => 'a-csharp 4567137'},
        "撤退中の艦は入渠出来ません"=>{ tr => "",      desc => "Strategy.DeckStateViews", ok => 'a-csharp 4567159'},
        "高速修復材を持っていません" =>{ tr => "",      desc => "Strategy.DeckStateViews", ok => 'a-csharp 4567187'},
        "この海域には入渠ドックがありません"=>{ tr => "",      desc => "UIShortCutMenu, Strategy.DeckStateViews", ok => 'a-csharp 4495932'},
        "入渠ドックに空きがありません"  =>{ tr => "",      desc => "Strategy.DeckStateViews", ok => 'a-csharp 4567215'},
        "艦隊の前方にうずしおが発生しました！"=>{ tr => "",      desc => "SortieMap.Defines", ok => ['a-csharp 4563245', 'a-csharp 4698597']},
        "{0}x{1}\nを入手しました！"=>{ tr => "",      desc => "SortieMap.Defines", ok => [ 'a-csharp 4563211', 'a-csharp 4698634']},
        "{0}x{1}を\n落としてしまいました…。"=>{ tr => "",      desc => "SortieMap.Defines", ok => ['a-csharp 4563367','a-csharp 4698667']},
        "{0}x{1}を\n落としてしまいました…。\n(電探が役立って、被害を抑えられた！)"=>{ tr => "",      desc => "SortieMap.Defines", ok => ['a-csharp 4563283','a-csharp 4698710',]},
        "気のせいだった。"=>{ tr => "",      desc => "SortieMap.Defines",ok=>['a-csharp 4565013','a-csharp 4698818']},
        "敵影を見ず。"=>{ tr => "",      desc => "SortieMap.Defines",},
        ...
        "水上偵察機による\n航空偵察を実施します。"=>{ tr => "",      desc => "SortieMap.Defines", ok => ['a-csharp 4565165', 'a-csharp 4698911']},
        "大型飛行艇による\n航空偵察を実施します。"=>{ tr => "",      desc => "SortieMap.Defines", ok => ['a-csharp 4565123', 'a-csharp 4698952']},
        "航空偵察予定地点に到着しましたが、\n稼働偵察機がないため、偵察を中止します。"=>{ tr => "",      desc => "SortieMap.Defines", ok => ['a-csharp 4565045', 'a-csharp 4698993']},
        "艦隊に洋上補給を行います。"=>{ tr => "",      desc => "SortieMap.Defines", ok => ['a-csharp 4563903', 'a-csharp 4699070']},
        "艦隊針路\n選択可能!"=>{ tr => "",      desc => "SortieMap.Defines", ok => ['a-csharp 4563503', 'a-csharp 4699097']},
